...
Each workflow is implemented as a Jupyter notebook [@kluyver2016jupyter] and bundled with example data (images and, where appropriate, segmentation masks) so it can be run without specialized preprocessing. The workflows are written in Python and built on widely used scientific and machine-learning libraries, including NumPy [@harris2020numpy], SciPy [@virtanen2020scipy], scikit-image [@van2014scikit], Matplotlib [@hunter2007matplotlib], scikit-learn [@pedregosa2011scikit], OpenCV [@opencv_library], TensorFlow [@tensorflow2015-whitepaper], and Keras [@chollet2015keras]. For ease of access, all notebooks can be launched directly in the browser via Binder [@jupyter_binder_2018], requiring no local installation. All materials are archived with a Zenodo DOI to ensure long-term accessibility [@jones2025batteryxctworkflows]. We hope to extend this offering in future versions to include further ML models for segmentation and denoising tasks, additional cell formats (pouch, prismatic), and further QA workflows, such as uncertainty reporting and demonstrating integration into larger data processing pipelines. By lowering the barrier to quantitative XCT analysis, these notebooks promote transparent, reproducible practice in battery science and provide adaptable templates for related problems in tomography and X-ray imaging.

# Statement of need

Although XCT is increasingly used to visualize battery structures and assess manufacturing quality, reproducible analysis workflows remain scarce. New users often stop at qualitative inspection, and many depend on proprietary software with limited documentation. This fragmentation complicates comparisons across studies and slows the adoption of best practices.
...
![Ellipse fitted to the canister in a radial slice of a Li-ion cylindrical cell](fig2.png)  
