...
This submission provides a set of open Jupyter notebooks that demonstrate analysis workflows for battery XCT data. The workflows target three quality-assurance tasks relevant to cylindrical cells:
...
- unrolling 2D slices of “jelly rolls” into polar coordinates (radius, angle) [@kok2019virtual] and fitting an ideal spiral to quantify deviations from the expected winding pattern [@sun2025health].
...
The notebooks presented here address this gap with accessible, open, and well-documented workflows for common battery QA tasks. They are intended for researchers and engineers who wish to:

- quantify electrode overhang in cylindrical cells via a range of geometric measures;
- derive canister metrics (e.g., diameter, eccentricity) and detect dents through ellipse fitting; and
- unwrap cylindrical “jelly rolls” into polar coordinates and fit an ideal spiral to evaluate departures from the expected geometry.

The contribution is a set of complete, reproducible analysis workflows rather than a standalone software library. Because XCT analysis typically involves iterative exploration, visualization, and interpretation, Jupyter notebooks are a natural format: they combine executable code, results, and narrative explanation in one place. By sharing these notebooks, we aim to make quantitative XCT analysis easier to understand, adapt, and extend in both battery research and the broader tomography community.
